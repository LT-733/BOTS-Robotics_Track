...
def arms_up(r_left, r_right, w_left, w_right):
    def left_arm_up():
        pass
...
def hands_out(r_left, r_right, w_left, w_right):
    LEFT_STUCK = 1
    LEFT_LOOSE = 0.2
    RIGHT_STUCK = -1   # mirrored
    RIGHT_LOOSE = -0.2   # mirrored
    HAND_OFFSET_STEP = 0.01
    def left_hand_move() -> None:
        left_hand_pos: float = float(r_left['pos'][5])
        vertical_torque_left: float|None = float(r_left['pos'][0]) if r_left.ready() else None
        if vertical_torque_left > LEFT_STUCK:
            print("left stuck")
            left_hand_pos -= HAND_OFFSET_STEP
        elif vertical_torque_left < LEFT_LOOSE:
            print("left loose")
            left_hand_pos += HAND_OFFSET_STEP
        w_left['pos'][5] = left_hand_pos
    def right_hand_move() -> None:
        right_hand_pos: float = float(r_right['pos'][5])
        vertical_torque_right: float|None = float(r_right['pos'][0]) if r_right.ready() else None
        if vertical_torque_right < RIGHT_STUCK:
            print("left stuck")
            right_hand_pos += HAND_OFFSET_STEP
        elif vertical_torque_right > RIGHT_LOOSE:
            print("left loose")
            right_hand_pos -= HAND_OFFSET_STEP
        w_right['pos'][5] = right_hand_pos
    

            

# def left_arm_up(hug_left, r_left, w_left):
#     left_hand_offset = hug_left[5]
#     while hug_left[0] > 0:
#         hug_left[0] -= 0.0025
#         vert_torque_left = float(r_left.data['torque'][0]) if r_left.ready() else None

#         if vert_torque_left is not None:
#             if vert_torque_left > LEFT_STUCK:
#                 print("Left stuck")
#                 left_hand_offset -= HAND_OFFSET_STEP
#             elif vert_torque_left < LEFT_LOOSE:
#                 print("Left Loose")
#                 left_hand_offset += HAND_OFFSET_STEP

#         pos = np.array(hug_left, dtype=np.float32)
#         pos[5] += left_hand_offset
#         w_left['pos'] = pos
#         time.sleep(0.005)

# def right_arm_up(hug_right, r_right, w_right):
#     right_hand_offset = hug_right[5]
#     while hug_right[0] < 0:
#         hug_right[0] += 0.0025
#         vert_torque_right = float(r_right.data['torque'][0]) if r_right.ready() else None

#         if vert_torque_right is not None:
#             if vert_torque_right < RIGHT_STUCK:
#                 right_hand_offset += HAND_OFFSET_STEP
#             elif vert_torque_right > RIGHT_LOOSE:
#                 right_hand_offset -= HAND_OFFSET_STEP

#         pos = np.array(hug_right, dtype=np.float32)
#         pos[5] += right_hand_offset
#         w_right['pos'] = pos
#         time.sleep(0.005)
...
def main():
    with (
        ...
        thread_arm_up = threading.Thread(target=arms_up, args=(r_left, r_right, w_left, w_right))
